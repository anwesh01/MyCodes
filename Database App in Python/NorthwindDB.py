...
class NorthwindDB:
    ...
    def insert_order(self, ProductId, Quantity, Discount, CustomerId, EmployeeId, OrderDate,RequiredDate, ShippedDate, 
        ...
        if emp == True and shipper == True and prodId == True and custId == True:
            ...
            # Due to foreign key constraints, value will be first inserted in Orders table and then in the order_details table.
    ...
    def delete_Order(self, OrderId):
        # Due to foreign key constraints, data will be first deleted from order_details table and then in the orders table.
    ...
    def restock_parts(self, ProductId, Quantity):
        # First, checking if the ProductId is valid or not.
        query = "SELECT ProductId FROM products"
        self.cursor.execute(query)
        records = self.cursor.fetchall()
        val = False
    ...
    def ship_order(self, OrderId):
        # First, checking if the OrderId is valid or not.
        query = "SELECT OrderId FROM orders"
        self.cursor.execute(query)
        records = self.cursor.fetchall()
        val = False
